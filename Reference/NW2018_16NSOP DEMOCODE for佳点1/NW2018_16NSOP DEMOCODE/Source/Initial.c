...
void Sub_CMPx_Initialation (void)
{
	...
	//		0：正相输入电压 < 反相输入电压
	//		1：正相输入电压 > 反相输入电压
	_opac0 = 0b10000000;			//--OPA Enable;  外部反馈电路
...
void SYS_INITIAL()
{
	...
	*/
	...
	_wdtc=0x0af;					// WDT 溢出周期 为 2^18/fLIRC
...
void IO_INITIAL()
{
	...
	*/
	...
	//_ctrl=0b00111000;				// RX 选择复用在 PB1 ; SCL 复用在PA7 ; SDA 复用在PA6
	...
	*/
	...
	_pas0=0b00111110;			//PA0 为 SDA，PA1为运放输出脚以及电流采样AN1，PA2为 SCL，PA3 为 STB 输出口
	_pas1=0b11110011;			// PA7/AN6 只为 IGBT 温度采样（和蜂鸣器复用），PA6/AN5 为面板温度采用，PA5 为风扇输出口，PA4/AN3 为VR采样

	_papu=0x0d;				//PA0~PA2~PA3口上拉电阻使能
	_pac=0xdd;					//pa口输入输出控制 0：输出 1：输入

	_pac5=0;					//pa5为风扇引脚，置为输出//
	_pa5=0;					//初始化pa5为低电平//
	...
	_pcs10=1;
	*/
	_pcs0=0b10111111; 			 // SYN-; IS0, AN7, IS1 is defined//pc3比较器0反向输入（同步电路），pc2比较器4同向输入（电流浪涌）
							//pc1电压检测，ad ，pc0运算放大器输入
	_pcs1=0b00000011;			 // SYN+ is defined//PC4比较器0和比较器1同向输入

	_pcpu=0x00;				//pc口上拉电阻使能 0：除能 1：使能
	_pcc=0x1f; 				//pa口输入输出控制 0：输出 1：输入
	...
	_pbs00=1;
	*/
	//_pbs0=0x00;				//pb口在16pin上没有
	//_pbc=0x07;				// pb口输出初始为输出0
	//_pb=0x07;
...
void	ADC_INITIAL()
{
	...
	*/
	...
	_sadc0=0b00100000;			// 初始化开启AD功能, 结果采用高8低4结构
...
void	CTM0_INITIAL()
{
	...
	*/
	...
	_ctm0c1=0x0c1;				// 定时计数模式;  CCRA匹配;  定时时间 500*4/16MHz = 125uS
...
void	PTM0_INITIAL()
{
	...
	*/
	...
	_ptm0rpl=500&0xff;  			//
	_ptm0rph=(500/256)&0x03;  		//CCRP周期: 500*Fctm;

	_ptm0c0=0x20;				// PTM0时钟： Fsys/16   //PTM0时钟： Fsys/64
	_ptm0c1=0x0c1;				//  定时计数模式;  CCRA匹配;  定时时间 1000*16/16MHz = 4mS//1000*64/16MHz = 4mS

	_pt0on=1;					//  启动定时计数

	_ptm0pe=0;				//  PTM0 CCRP中断功能关闭
	_ptm0ae=1;				//  PTM0 CCRA中断功能开启
	_mf0e=1;					//  多功能中断0 开启.
}
